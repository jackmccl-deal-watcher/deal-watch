...
def populate_cpus():
    ...
    for cpu in cpu_data:
        cpu['brand'] = cpu['name'].split(' ', 1)[0]
        cpu['model'] = cpu['name'].split(' ', 1)[1]
        cpu_doc = {
            "type": "cpu",
            "model": cpu['model'],
            "brand": cpu['brand'],
            "cores": cpu['core_count'],
            "base_clock": ghz_to_hz(cpu['core_clock']),
            'boost_clock': 0,
        ...
        if (cpu['price']):
            cpu_doc['pcpp_price'] = float(cpu['price'])
        else:
            cpu_doc['pcpp_price'] = float(0)

        # Uses base_clock as boost_clock if no boost_clock
        if cpu['boost_clock']:
            cpu_doc["boost_clock"] = ghz_to_hz(cpu['boost_clock'])
        else:
            cpu_doc["boost_clock"] = ghz_to_hz(cpu['core_clock'])
