...
const VideoCardForm = ({ handlePartEvaluation }) => {
    ...
    return(
        <div className='component-form'>
            ...
            <Slider min={125000000} max={48000000000} valueLabelDisplay='auto' valueLabelFormat={getVramLabelText} value={vram} onChange={(e, newValue) => setVram(newValue)}></Slider>
            <p className='component-form-input-label'>Base Clock: {getClockSpeedLabelText(baseClock)}</p>
            <Slider min={115000000} max={1825000000} valueLabelDisplay='auto' valueLabelFormat={getClockSpeedLabelText} value={baseClock} onChange={(e, newValue) => setBaseClock(newValue)}></Slider>
            <p className='component-form-input-label'>Boost Clock: {getClockSpeedLabelText(boostClock)}</p>
            <Slider min={250000000} max={2250000000} valueLabelDisplay='auto' valueLabelFormat={getClockSpeedLabelText} value={boostClock} onChange={(e, newValue) => setBoostClock(newValue)}></Slider>
