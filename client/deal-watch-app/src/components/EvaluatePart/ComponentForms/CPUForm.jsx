...
const CPUForm = ({ handlePartEvaluation }) => {
    ...
    return(
        <div className='component-form'>
            ...
            <Slider min={1} max={64} valueLabelDisplay='auto' valueLabelFormat={getCoresLabelText} value={cores} onChange={(e, newValue) => setCores(newValue)}></Slider>
            <p className='component-form-input-label'>Base Clock: {getClockSpeedLabelText(baseClock)}</p>
            <Slider min={1000000000} max={4800000000} valueLabelDisplay='auto' valueLabelFormat={getClockSpeedLabelText} value={baseClock} onChange={(e, newValue) => setBaseClock(newValue)}></Slider>
            <p className='component-form-input-label'>Boost Clock: {getClockSpeedLabelText(boostClock)}</p>
            <Slider min={1000000000} max={5500000000} valueLabelDisplay='auto' valueLabelFormat={getClockSpeedLabelText} value={boostClock} onChange={(e, newValue) => setBoostClock(newValue)}></Slider>
